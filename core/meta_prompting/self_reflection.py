...
def self_reflect(response: str) -> Dict[str, Any]:
    ...
    scores["usefulness"] = 2 if any(w in response.lower() for w in ["should", "recommend", "suggest", "implement"]) else 1
    comments["usefulness"] = "Actionable advice detected." if scores["usefulness"] == 2 else "Could be more actionable."
    # Tone: check for professionalism
    scores["tone"] = 2 if all(w not in response.lower() for w in ["dumb", "stupid", "idiot"]) else 1
    comments["tone"] = "Professional tone." if scores["tone"] == 2 else "Unprofessional language detected."
    # Logical consistency: check for contradiction keywords
    scores["logical_consistency"] = 2 if "however" not in response.lower() else 1
    comments["logical_consistency"] = "No obvious contradictions." if scores["logical_consistency"] == 2 else "Possible contradiction detected."
