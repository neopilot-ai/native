import pytest
from core.tool_chain.executor import ToolExecutor, PermissionError
...
def test_execute_chain():
    ...
    def increment(x):
        return x + 1
    executor = ToolExecutor(user_roles=["admin"])
    executor.register_tool("double", double, description="Double a number.", permissions=["admin"])
    executor.register_tool("increment", increment, description="Increment a number.", permissions=["admin"])
    result = executor.execute_chain(["double", "increment"], initial_input=3)
    assert result == 7  # (3*2)+1

def test_shell_tool():
    executor = ToolExecutor(user_roles=["admin"])
    executor.register_shell_tool("echo", "echo {args}", description="Echo input.", permissions=["admin"])
