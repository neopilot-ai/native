...
from prompting.system_prompts.principal_engineer_prompt import load_system_prompt, build_combined_prompt
...
def main(
    ...
    feedback: bool = typer.Option(False, "--feedback", help="Enable feedback/counter-feedback loop"),
    ...
    combined_prompt = build_combined_prompt(prompt, system_prompt_path)
...
@app.command()
def memory_viewer(
    path: str = typer.Option("data/memory_store.json", help="Path to memory store JSON"),
    ...
    if search:
        data = [entry for entry in data if search.lower() in entry.get("prompt", "").lower()]
    total = len(data)
    if total == 0:
    ...
    typer.echo(f"Showing {start+1}-{min(end, total)} of {total} results (Page {page})\n")
